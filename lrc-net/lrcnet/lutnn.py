...
class LUTNN(nn.Module):
    def __init__(self, luts_per_layer, num_layers, lut_size, input_dim: int, num_classes: int, tau=10, 
        ...
        layers = []
        if connections == 'GRC' or 'LRC':
            layers.append(nn.Flatten())
            layers.append(
                LUTLayer(input_dim=input_dim, lut_size=lut_size[0], n_luts=luts_per_layer[0], radius = 5, 
                         device=device, dataset=dataset, connections=connections,  init_type=init_type))
            for i in range(1, num_layers):
                layers.append(LUTLayer(input_dim=luts_per_layer[i - 1], lut_size=lut_size[i], n_luts=luts_per_layer[i], radius = 2, 
                                    device=device, dataset=dataset, connections='GRC', init_type=init_type))                     
